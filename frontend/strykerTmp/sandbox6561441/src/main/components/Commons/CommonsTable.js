...
export default function CommonsTable({
  commons,
  currentUser
}) {
  if (stryMutAct_9fa48("90")) {
    ...
    const editCallback = cell => {
      if (stryMutAct_9fa48("91")) {
        ...
        stryCov_9fa48("91");
        navigate(stryMutAct_9fa48("92") ? `` : (stryCov_9fa48("92"), `/admin/editcommons/${cell.row.values.id}`));
      }
    ...
    const leaderboardCallback = cell => {
      navigate(`/admin/showleaderboard/${cell.row.values.id}`);
    };
    ...
    const columnsIfAdmin = [...columns, ButtonColumn("Leaderboard", "secondary", leaderboardCallback, testid), ButtonColumn("Edit", "primary", editCallback, testid), ButtonColumn("Delete", "danger", deleteCallback, testid)];
